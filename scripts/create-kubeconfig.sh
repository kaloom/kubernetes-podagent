...
kubecfg_file=${2:-podagent-kubeconfig.yaml}
...
kubectl apply -f podagent-serviceaccount-and-rbac.yaml
# get the secret name from the service account
secret_name=$(kubectl get sa podagent -n $namespace -o jsonpath="{.secrets[*].name}")
...
kubectl config set-credentials "podagent-${cluster_name}" --kubeconfig="${kubecfg_path}" --token="${token}"

# Set a context entry in kubeconfig
kubectl config set-context "podagent-${cluster_name}" --kubeconfig="${kubecfg_path}" --cluster="${cluster_name}" --user="podagent-${cluster_name}"

# Set the current-context in the kubeconfig file
kubectl config use-context "podagent-${cluster_name}" --kubeconfig="${kubecfg_path}"
