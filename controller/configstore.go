...
const (
	defaultConfigDir = "/var/run/podagent/"
)

type ExpectedState string

const (
	Add    ExpectedState = "Add"
	Delete ExpectedState = "Delete"
)
...
func (cs *ConfigStore) getConfigRecordKey(podName, networkName string) string {
	return fmt.Sprintf("%s-%s", podName, networkName)
}
...
func (cs *ConfigStore) saveRunningConfig(key string, running RunningConfig) error {
	glog.V(5).Infof("Saving running config:%+v, with Key: %s", running, key)
	cs.mu.Lock()
	defer cs.mu.Unlock()
...
func (cs *ConfigStore) saveExpectedConfig(key string, expected ExpectedConfig) error {
	glog.V(5).Infof("Saving expected config:%+v, with Key: %s", expected, key)
	cs.mu.Lock()
	defer cs.mu.Unlock()
...
func (cs *ConfigStore) getConfigRecord(key string) (ConfigRecord, error) {
	...
	glog.V(5).Infof("Returning configRecord:%+v, with Key: %s", currConfigRec, key)
	return currConfigRec, nil
}

func (cs *ConfigStore) delConfigRecord(key string) error {
	glog.V(5).Infof("Deleting configRecord with Key: %s", key)
	cs.mu.Lock()
	defer cs.mu.Unlock()
	path := filepath.Join(cs.dir, key)
